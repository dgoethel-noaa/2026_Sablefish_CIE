# Purpose: Spatial Sablefish Sensitivity Run - Multinomial Release Conditioned (5 region model)
...
dat_new_2025 <- readRDS(here("Mods", "Spat_Cont", "Spat_5_Reg", "SPoRC_Spatial_5_Regions.rds"))
...
saveRDS(sabie_rtmb_model, here("Mods", "Spat_Cont", "Spat_5_Reg", "5_rg_MltRel.RDS"))

sabie_rtmb_model <- readRDS(here("Mods", "Spat_Cont", "Spat_5_Reg", "5_rg_MltRel.RDS"))
data <- sabie_rtmb_model$data

pdf(here("Mods", "Spat_Cont", "Spat_5_Reg", "figs", "main_res.pdf"), width = 19, height = 10)
...
lens <- data$lens
years <- data$years
comp_regions <- data.frame(Region = 1:5, Region_Name = c("BS", 'AI', 'WGOA', 'CGOA', "EGOA"))
...
pdf(here("Mods", "Spat_Cont", "Spat_5_Reg", "figs", 'llf_acomp_fits.pdf'), width = 19, height = 10) 
...
### Fishery Lengths ------------------------------------------------------------
#### LLF ---------------------------------------------------------------------
pdf(here("Mods", "Spat_Cont", "Spat_5_Reg", "figs", 'llf_lcomp_fits.pdf'), width = 19, height = 10) 
for(r in 1:nrow(comp_regions)) {
  for(s in 1:nrow(sex_names)) {
...
#### Trawl ---------------------------------------------------------------------
pdf(here("Mods", "Spat_Cont", "Spat_5_Reg", "figs", 'twlf_lcomp_fits.pdf'), width = 19, height = 10) 
for(r in 1:nrow(comp_regions)) {
  for(s in 1:nrow(sex_names)) {
...
pdf(here("Mods", "Spat_Cont", "Spat_5_Reg", "figs", 'domll_acomp_fits.pdf'), width = 19, height = 10) 
for(r in 1:nrow(comp_regions)) {
  for(s in 1:nrow(sex_names)) {
...
pdf(here("Mods", "Spat_Cont", "Spat_5_Reg", "figs", 'jpll_acomp_fits.pdf'), width = 19, height = 10) 
for(r in 1:nrow(comp_regions)) {
  for(s in 1:nrow(sex_names)) {
...
pdf(here("Mods", "Spat_Cont", "Spat_5_Reg", "figs", 'retro.pdf'), width = 13, height = 10) 
retro_plot
dev.off()
