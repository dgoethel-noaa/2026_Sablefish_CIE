...
mod_name <- '26.1i_1Reg_Cont_Upd_No_wAI_LLS'                                                    # Model name to use for saving files and labels
...
SPoRC_one_reg <- readRDS(here(root_folder,"26.1","SPoRC_1reg_joint_JPN_len.rds"))
...
write.csv(sd_summ,file = here(root_folder,'26.1',paste0(mod_name,"_sd_rep.csv")))
...
write.csv(naa_tab,file = here(root_folder,'26.1',paste0(mod_name,"_NAA.csv")))
...
SE_tab <- rbind(SE_tab_LLS_mean, SE_tab_JPN_LLS_mean)
...
if(do_mcmc == 1) {
  ...
  mod <- readRDS(file = here(root_folder,'26.1', paste0(mod_name,"_model_results.RDS")))
  ...
  # save MCMC
  saveRDS(mcmc, here('Mods', '___Final_Mods', "26.1", paste(mod_name, "mcmc.RDS", sep = '_')))
  
  # plot mcmc
  pdf(here('Mods', '___Final_Mods', "26.1", "plots", paste(mod_name, "mcmc.pdf", sep = '_')))
  SparseNUTS::plot_marginals(mcmc, pars = c('ln_global_R0', 'ln_srv_q[1]',
                                            'ln_srv_q[2]', 'ln_M[1]', 'ln_M[2]'))
  pairs(mcmc, pars = c('ln_global_R0', 'ln_srv_q[1]',
                       'ln_srv_q[2]', 'ln_M[1]', 'ln_M[2]'))
...
mcmc <- readRDS(here('Mods', '___Final_Mods', "26.1", paste(mod_name, "mcmc.RDS", sep = '_')))
...
mcmc_comp_ts_table = read.csv( here(root_folder,mod_name, paste0(mod_name,"_mcmc_ts.csv")))

pdf(here('Mods', '___Final_Mods', "26.1", "plots", paste(mod_name, "mcmc_ts.pdf", sep = '_')))
...
mcmc_comp_par_table <- mcmc$monitor %>% 
  filter(str_detect(variable, 'R0|srv_q|M|SSB')) %>% 
  left_join(
    data.frame(
      variable = c('ln_global_R0', 'ln_M[1]', 'ln_M[2]', 'ln_srv_q[1]', 'ln_srv_q[2]'),
...
write.csv(mcmc_comp_par_table, here(root_folder,'26.1', paste0(mod_name,"_mcmc_par.csv")))
...
write.csv(proj_table, here(root_folder,'26.1', paste0(mod_name,"_proj_table.csv")))
write.csv(proj_table2, here(root_folder,'26.1', paste0(mod_name,"_proj_table2.csv")))
...
write.csv(all_scenarios_bio2, here(root_folder,'26.1', paste0(mod_name,"_proj_table_bio.csv")))
...
pdf(here('Mods', '___Final_Mods', "26.1", "plots", paste(mod_name, "phase_plane.pdf", sep = '_')))
...
pdf(here('Mods', '___Final_Mods', "26.1", "plots", paste(mod_name, "ssb_rec.pdf", sep = '_')))
...
est_pars <- data.frame(name = names(sabie_rtmb_model$sd_rep$par.fixed), 
  ...
  count() %>% 
  rename(n_pars = n)
write.csv(est_pars, here(root_folder,'26.1', paste0(mod_name,"_par_count_table.csv")))
