...
mod_name2 <- c("25.12_Drop_TS_Upd_M_2026","25.12_1Reg_LLS_Carry","25.12_1Reg_LLS_Carry_no_wAI","25.12_1Reg_Surv_CV_1.5",
               "25.12_1Reg_No_CPUE","25.12_1Reg_Sex_M","25.12_1Reg_Upd_Growth_WAA","25.12_1Reg_2_Blks_Fsh_No_Len")                         # names of model want to compare, these must match the names used for the SPoRC outputs results rdata files
comp_name <- "_Final_one_off"                                                       # name for saving files

path <- here("Mods","_1Reg_Comp",comp_name)                   # Assumes that comparison folder is inside the first model folder, but can update this to be elsewhere

do_high_res_plots <- 0

# extract out stuff------------------------------------------------------------------

info <- readRDS(here(path,"SPoRC_1reg_joint_JPN_len.rds"))                            # read in data file to get years
...
rep[[3]]$ln_sigmaC <- rep[[1]]$ln_sigmaC
rep[[4]]$ln_sigmaC <- rep[[1]]$ln_sigmaC
rep[[2]]$ln_sigmaC <- rep[[1]]$ln_sigmaC

rep[[2]]$NAA0 <- rep[[1]]$NAA0
rep[[3]]$NAA0 <- rep[[1]]$NAA0
rep[[4]]$NAA0 <- rep[[1]]$NAA0

rep[[3]]$sexratio <- rep[[1]]$sexratio
rep[[4]]$sexratio <- rep[[1]]$sexratio
rep[[2]]$sexratio <- rep[[1]]$sexratio
