...
mod_name <- '25.12_1Reg_TF_log'                                                    # Model name to use for saving files and labels
root_folder <- here("Mods", "1Reg_Selex")                                            # The root folder where runs will be stored (sub folders named with mod_name)
...
########################################################################################################################
#input_list_gg_mar = readRDS(here("Mods","Base","25.12_1Reg_Cont","Feb_Results","SPoRC_one_reg_age_joint_no_len_kp_JPN.rds"))
#data_gg_mar = input_list_gg_mar
#SPoRC_one_reg$UseSrvIdx <- data_gg_mar[["UseSrvIdx"]]
#SPoRC_one_reg$ObsSrvIdx <- data_gg_mar[["ObsSrvIdx"]]
#SPoRC_one_reg$ObsSrvIdx_SE <- data_gg_mar[["ObsSrvIdx_SE"]]
...
input_list$map$ln_fish_fixed_sel_pars <- factor(c(1:7, 2, 8:11,                # sharing logistic delta across sexes from early LLF fishery (first time block), all other pars estimated for LLF across all time blocks
                                                  ...
                                                  rep(c(14,15),3)))            # for trawl fishery males, share the bmax par of the gamma function with the female estimate par (repeat 3 times to fill the 3 time blocks associated with the LLF)
