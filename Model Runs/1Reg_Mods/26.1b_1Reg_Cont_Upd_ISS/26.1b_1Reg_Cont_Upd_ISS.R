...
mod_name <- '26.1b_1Reg_Cont_Upd_ISS'                                                    # Model name to use for saving files and labels
root_folder <- here("Mods", "_1Reg_Mod_Build")                                            # The root folder where runs will be stored (sub folders named with mod_name)
...
Wt_FishAgeComps[1,,1,1] <- 1                                   # Weight for fixed gear aggregated age comps
Wt_FishLenComps[1,,1,1] <- 1                                     # Weight for fixed gear len comps females
Wt_FishLenComps[1,,2,1] <- 1                                    # Weight for fixed gear len comps males
Wt_FishLenComps[1,,1,2] <- 1                                   # Weight for trawl gear len comps females
Wt_FishLenComps[1,,2,2] <- 1                                   # Weight for trawl gear len comps males

Wt_SrvAgeComps[1,,1,1] <- 1                                     # Weight for domestic survey ll gear age comps
Wt_SrvAgeComps[1,,1,3] <- 1                                     # Weight for coop jp survey ll gear age comps
Wt_SrvLenComps[1,,1,1] <- 1                                     # Weight for domestic ll survey len comps females
Wt_SrvLenComps[1,,2,1] <- 1                                     # Weight for domestic ll survey len comps males
Wt_SrvLenComps[1,,1,2] <- 1                                    # Weight for domestic trawl survey len comps females
Wt_SrvLenComps[1,,2,2] <- 1                                    # Weight for domestic trawl survey len comps males
Wt_SrvLenComps[1,,1,3] <- 1                                     # Weight for coop jp ll survey len comps females
Wt_SrvLenComps[1,,2,3] <- 1                                    # Weight for coop jp ll survey len comps males
...
input_list <- Setup_Mod_Fishsel_and_Q(input_list = input_list,
                                      ...
                                      fish_q_blocks =                          # fishery indices catchability time blocks
                                        c(
                                          "Block_1_Year_1-35_Fleet_1",         # pre-IFQ time block for fixed gear fishery 1994 and before
                                          "Block_2_Year_36-56_Fleet_1",        # IFQ time block for fixed gear fishery-- 1995 to 2015
                                          "Block_3_Year_57-terminal_Fleet_1",  # Recent time block for fixed gear fishery--2016 to terminal year
...
# CONT SETUP
input_list$map$ln_fish_fixed_sel_pars <- factor(c(1:12,                # LLF 3 blocks, 2 sexes, no shared pars
                                                  rep(13:14,3),               # for trawl fishery estimate both female gamma pars, but no time blocks (repeat 3 times to fill the 3 time blocks associated with the LLF)
                                                  rep(c(15,16),3)))            # for trawl fishery males, estimate both pars (repeat 3 times to fill the 3 time blocks associated with the LLF)
...
input_list$map$ln_srv_fixed_sel_pars <-
  factor(c(1:8,                                                                # domestic ll survey (2 time blocks, 2 sexes),
           ...
           rep(c(11:12),2)))                                                  # coop JPN LL survey (single time block, males)
...
if(do_pearson == 1){
  ...
  comp_type = 2
  bin_label = "Ages"
  data_source = "LLS Age Compositions"
  ...
  lls_age_pear <- pearson_stuff[[2]]
  
  tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_Pearson_Resids_LLS_Age.png")),units = "in", width=12,height=8, res = 300)
  print(lls_age_pear)
  dev.off()
  ...
  if(any(data$UseSrvLenComps[,,1] == 1)){
    ...
    comp_type = 2
    bin_label = "Lengths"
    data_source = "LLS Length Compositions"
    ...
    lls_len_pear <- pearson_stuff[[2]]
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_Pearson_Resids_LLS_Length.png")),units = "in", width=12,height=8, res = 300)
    print(lls_len_pear)
    dev.off()
  }
...
ISS_LLF_len = c(na.omit(data$ISS_FishLenComps[1,which(data$UseFishLenComps[,,1] == 1),1,1]))

wt_LLF_len = c(na.omit(unique(data$Wt_FishLenComps[1,which(data$UseFishLenComps[,,1] == 1),1,1])))

ESS_LLF_len = c(na.omit(data$ISS_FishLenComps[1,which(data$UseFishLenComps[,,1] == 1),1,1] * 
                          unique(data$Wt_FishLenComps[1,which(data$UseFishLenComps[,,1] == 1),1,1])))

ESS_tab_LLF_len <- data.frame(ISS = ISS_LLF_len, Data_Wt = rep(wt_LLF_len,length(ISS_LLF_len)), ESS = ESS_LLF_len) 
write.csv(ESS_tab_LLF_len, here(root_folder,mod_name,"plots",paste0(mod_name,"_LLF_ESS_Len.csv")))

ESS_tab_LLF_len_mean <- ESS_tab_LLF_len %>% summarize(ISS = mean(ISS), Data_Wt = mean(Data_Wt), ESS = mean(ESS)) %>% 
  mutate(Source = "Fixed Gear Fish Lengths") %>% dplyr::relocate(Source) %>%
  mutate(dplyr::across(where(is.numeric), round, 2))
...
ISS_LLS_age = c(na.omit(data$ISS_SrvAgeComps[1,which(data$UseSrvAgeComps[,,1] == 1),1,1]))

wt_LLS_age = c(na.omit(unique(data$Wt_SrvAgeComps[1,which(data$UseSrvAgeComps[,,1] == 1),1,1])))

ESS_LLS_age = c(na.omit(data$ISS_SrvAgeComps[1,which(data$UseSrvAgeComps[,,1] == 1),1,1] * 
                          unique(data$Wt_SrvAgeComps[1,which(data$UseSrvAgeComps[,,1] == 1),1,1])))

ESS_tab_LLS_Age <- data.frame(ISS = ISS_LLS_age, Data_Wt = rep(wt_LLS_age,length(ISS_LLS_age)), ESS = ESS_LLS_age) 
write.csv(ESS_tab_LLS_Age, here(root_folder,mod_name,"plots",paste0(mod_name,"_LLS_ESS_Age.csv")))

ESS_tab_LLS_Age_mean <- ESS_tab_LLS_Age %>% summarize(ISS = mean(ISS), Data_Wt = mean(Data_Wt), ESS = mean(ESS)) %>% 
  mutate(Source = "LL Survey Ages") %>% dplyr::relocate(Source) %>%
  mutate(dplyr::across(where(is.numeric), round, 2))

ISS_LLS_len = c(na.omit(data$ISS_SrvLenComps[1,which(data$UseSrvLenComps[,,1] == 1),1,1]))

wt_LLS_len = c(na.omit(unique(data$Wt_SrvLenComps[1,which(data$UseSrvLenComps[,,1] == 1),1,1])))

ESS_LLS_len = c(na.omit(data$ISS_SrvLenComps[1,which(data$UseSrvLenComps[,,1] == 1),1,1] * 
                          unique(data$Wt_SrvLenComps[1,which(data$UseSrvLenComps[,,1] == 1),1,1])))

ESS_tab_LLS_len <- data.frame(ISS = ISS_LLS_len, Data_Wt = rep(wt_LLS_len,length(ISS_LLS_len)), ESS = ESS_LLS_len) 
write.csv(ESS_tab_LLS_len, here(root_folder,mod_name,"plots",paste0(mod_name,"_LLS_ESS_Len.csv")))

ESS_tab_LLS_len_mean <- ESS_tab_LLS_len %>% summarize(ISS = mean(ISS), Data_Wt = mean(Data_Wt), ESS = mean(ESS)) %>% 
  mutate(Source = "LL Survey Lengths") %>% dplyr::relocate(Source) %>%
  mutate(dplyr::across(where(is.numeric), round, 2))

ISS_JPN_LLS_age = c(na.omit(data$ISS_SrvAgeComps[1,which(data$UseSrvAgeComps[,,3] == 1),1,3]))

wt_JPN_LLS_age = c(na.omit(unique(data$Wt_SrvAgeComps[1,which(data$UseSrvAgeComps[,,3] == 1),1,3])))

ESS_JPN_LLS_age = c(na.omit(data$ISS_SrvAgeComps[1,which(data$UseSrvAgeComps[,,3] == 1),1,3] * 
                              unique(data$Wt_SrvAgeComps[1,which(data$UseSrvAgeComps[,,3] == 1),1,3])))
...
ISS_JPN_LLS_len = c(na.omit(data$ISS_SrvLenComps[1,which(data$UseSrvLenComps[,,3] == 1),1,3]))

wt_JPN_LLS_len = c(na.omit(unique(data$Wt_SrvLenComps[1,which(data$UseSrvLenComps[,,3] == 1),1,3])))

ESS_JPN_LLS_len = c(na.omit(data$ISS_SrvLenComps[1,which(data$UseSrvLenComps[,,3] == 1),1,3] * 
                              unique(data$Wt_SrvLenComps[1,which(data$UseSrvLenComps[,,3] == 1),1,3])))
...
ESS_tab <- rbind(ESS_tab_LLF_Age_mean, ESS_tab_LLS_Age_mean, 
                 ESS_tab_JPN_LLS_Age_mean, ESS_tab_LLF_len_mean,
                 ESS_tab_TF_len_mean, ESS_tab_LLS_len_mean, ESS_tab_JPN_LLS_Len_mean)



SE_LLS_idx = c(na.omit(data$ObsSrvIdx_SE[1,which(data$UseSrvIdx[,,1] == 1),1]))

wt_LLS_idx = data$Wt_SrvIdx

ESE_LLS_idx = c(na.omit(data$ObsSrvIdx_SE[1,which(data$UseSrvIdx[,,1] == 1),1] * 
                          data$Wt_SrvIdx))

SE_tab_LLS_idx <- data.frame(SE = SE_LLS_idx, Data_Wt = rep(wt_LLS_idx,length(SE_LLS_idx)), Eff_SE = ESE_LLS_idx) 
write.csv(SE_tab_LLS_idx, here(root_folder,mod_name,"plots",paste0(mod_name,"_LLS_Idx_SE.csv")))

SE_tab_LLS_mean <- SE_tab_LLS_idx %>% summarize(SE = mean(SE), Data_Wt = mean(Data_Wt), Eff_SE = mean(Eff_SE)) %>% 
  mutate(Source = "LL Survey Index") %>% dplyr::relocate(Source) %>%
  mutate(dplyr::across(where(is.numeric), round, 2))

SE_JPN_LLS_idx = c(na.omit(data$ObsSrvIdx_SE[1,which(data$UseSrvIdx[,,3] == 1),3]))

wt_JPN_LLS_idx = data$Wt_SrvIdx

ESE_JPN_LLS_idx = c(na.omit(data$ObsSrvIdx_SE[1,which(data$UseSrvIdx[,,3] == 1),3] * 
                              data$Wt_SrvIdx))
...
SE_LLF_idx = c(na.omit(data$ObsFishIdx_SE[1,which(data$UseFishIdx[,,1] == 1),1]))

wt_LLF_idx = data$Wt_FishIdx

ESE_LLF_idx = c(na.omit(data$ObsFishIdx_SE[1,which(data$UseFishIdx[,,1] == 1),1] * 
                          data$Wt_FishIdx))

SE_tab_LLF_idx <- data.frame(SE = SE_LLF_idx, Data_Wt = rep(wt_LLF_idx,length(SE_LLF_idx)), Eff_SE = ESE_LLF_idx) 
write.csv(SE_tab_LLF_idx, here(root_folder,mod_name,"plots",paste0(mod_name,"_LLF_Idx_SE.csv")))

SE_tab_LLF_mean <- SE_tab_LLF_idx %>% summarize(SE = mean(SE), Data_Wt = mean(Data_Wt), Eff_SE = mean(Eff_SE)) %>% 
  mutate(Source = "Fixed Gear Fishery CPUE") %>% dplyr::relocate(Source) %>%
  mutate(dplyr::across(where(is.numeric), round, 2))

SE_tab <- rbind(SE_tab_LLS_mean, SE_tab_JPN_LLS_mean, SE_tab_LLF_mean)
