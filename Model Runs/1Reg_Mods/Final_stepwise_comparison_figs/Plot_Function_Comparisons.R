...
mod_name <- c("26.1i_1Reg_Cont_Upd_No_wAI_LLS",
              #"26.1h_1Reg_Cont_Upd_LLS_Carry",
              "26.1g_1Reg_Cont_Upd_No_Len_2_Blks_F",
              #"26.1f_1Reg_Cont_Upd_No_CPUE",
              "26.1e_1Reg_Cont_Upd_Sex_M",
              #"26.1d_1Reg_Cont_Upd_Bio",
              #"26.1c_1Reg_Cont_Upd_TF_log",
              #"26.1b_1Reg_Cont_Upd_ISS",
              "26.1a_1Reg_Cont_Upd",
              "25.12_1Reg_Cont")                         # names of model want to compare, these must match the names used for the SPoRC outputs results rdata files
# names of model want to compare, these must match the names used for the SPoRC outputs results rdata files
comp_name <- "1Reg_Mods"                                                       # name for saving files
root_folder <- "Mods"                                            # The root folder where runs will be stored (sub folders named with mod_name)

ass_path <- here(root_folder,"_1Reg_Comp","__Final_stepwise")                             # Assumes that comparison folder is inside the first model folder, but can update this to be elsewhere

do_high_res_plots <- 1

# extract out stuff------------------------------------------------------------------

info <- readRDS(here(ass_path,"SPoRC_1reg_joint_JPN_len.rds"))                            # read in data file to get years
...
idx_plot <- idx_fit+
  ...
  theme(title = element_text(size=30),axis.title = element_text(size = 18),legend.text = element_text(size = 20),legend.title = element_text(size = 18, face = "bold")) +
  geom_line(lwd=2)
...
M_plot <- bios[[2]]+
  ...
  theme(title = element_text(size=30),axis.title = element_text(size = 18),legend.text = element_text(size = 8),legend.title = element_text(size = 9, face = "bold")) +
...
f_selex <- selex[[1]]+
  ...
  theme(title = element_text(size=30),axis.title = element_text(size = 18),legend.text = element_text(size = 7),legend.title = element_text(size = 7, face = "bold"))
...
catch_plot <- catch + labs(title = "Catch (kt)")+
  theme(title = element_text(size=30),axis.title = element_text(size = 18),legend.text = element_text(size = 10),legend.title = element_text(size = 12, face = "bold")) +
  geom_line(linewidth=2)
...
F_plot_CI <- time_series[[2]]+  facet_wrap(~Type) +
  ...
  theme(title = element_text(size=30),axis.title = element_text(size = 18),legend.text = element_text(size = 18),legend.title = element_text(size = 20, face = "bold")) +
  geom_line(linewidth=2)
...
ts_plot <-      time_series[[1]] +
  ...
  ggplot2::scale_x_continuous(breaks = seq(1, length(mod_years), 5),
                              labels = (mod_start_year  - 1)+ seq(1, length(mod_years), 5)) + labs(title = "Time Series Plots")+
  theme(title = element_text(size=30),axis.title = element_text(size = 18),legend.text = element_text(size = 12),legend.title = element_text(size = 14, face = "bold")) +
  geom_line(linewidth=2)
...
data_plot <- data_used+
  ...
  theme(title = element_text(size=30),axis.title = element_text(size = 18),legend.text = element_text(size = 12),legend.title = element_text(size = 30, face = "bold"))
...
if(do_high_res_plots == 1){
  ...
  tiff(filename=here(ass_path,paste0(comp_name,"_M.png")),units = "in", width=12,height=8, res = 300)
  print(M_plot)
  dev.off()
  tiff(filename=here(ass_path,paste0(comp_name,"_WAA.png")),units = "in", width=12,height=8, res = 300)
  print(WAA_plot)
  dev.off()
  tiff(filename=here(ass_path,paste0(comp_name,"_Mat.png")),units = "in", width=12,height=8, res = 300)
  print(MAA_plot)
  dev.off()
