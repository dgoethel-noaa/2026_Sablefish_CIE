...
mod_name <- '26.2e_FAA_3Flt_log2domeS'                                                    # Model name to use for saving files and labels
...
SPoRC_one_reg <- readRDS(here(root_folder,mod_name,"SPoRC_FAA_3FG_1TF.rds"))
...
input_list <- Setup_Mod_FishIdx_and_Comps(input_list = input_list,
                                          ...
                                          FishAgeComps_Type =                  # age comp structure for fishery fleet 1 and 2 (agg_Year_1-terminal_Fleet_1 indicates that age comps are aggregated across sex for all years for fleet 1--fixed gear)
                                            c("spltRjntS_Year_1-terminal_Fleet_1",   # NOTE: these entries are needed for all years and all fleets, so if want to agg for some years then disagg for remaining years, would need two entries for that fleet
                                              ...
                                              "none_Year_1-terminal_Fleet_4"), # NOTE: if want joint by sex would use spltRjntS_Year_1-terminal_Fleet_1
                                          ...
                                          FishAge_comp_agg_type = c(1,1,1,NA),     # for age comps, identifies the sequence of the normalizing, aggregation, and aging error process (matches what is done in ADMB sablefish currently)
                                          FishLen_comp_agg_type = c(0,0,0,0)       # for length comps, identifies the sequence of the normalizing, aggregation, and aging error process (matches what is done in ADMB sablefish currently)
)
...
input_list <- Setup_Mod_SrvIdx_and_Comps(input_list = input_list,
                                         ...
                                         ObsSrvIdx_SE =                        # Observed fishery independent survey standard errors
                                           (SPoRC_one_reg$ObsSrvIdx_SE)*1.5,       # variance is BS~0.14, AI~0.12, GOA~0.05
...
fleet_blocks <- data.frame(
  fleet = c(1, 1, 2,2,3,3,4),                                           # fleets corresponding to time blocks (3 fixed gear time blocks, no trawl gear blocks)
  block = c(1, 2, 1, 2,1, 2,  1)                                       # corresponding time blocks
)
...
input_list <- Setup_Mod_Fishsel_and_Q(input_list = input_list,
                                      cont_tv_fish_sel =                       # whether continuous time-varying selectivity is used for either fishery fleet
                                        c("none_Fleet_1", "none_Fleet_2", "none_Fleet_3", "none_Fleet_4"),
                                      fish_sel_blocks =                        # fishery selectivity time blocks if not TV specified above for a given fleet
                                        c("Block_1_Year_1-56_Fleet_1",         # pre-IFQ time block for fixed gear fishery 1994 and before
                                          ...
                                          "Block_1_Year_1-56_Fleet_2",         # pre-IFQ time block for fixed gear fishery 1994 and before
                                          "Block_2_Year_57-terminal_Fleet_2",  # Recent time block for fixed gear fishery--2016 to terminal year
                                          "Block_1_Year_1-56_Fleet_3",         # pre-IFQ time block for fixed gear fishery 1994 and before
                                          "Block_2_Year_57-terminal_Fleet_3",  # Recent time block for fixed gear fishery--2016 to terminal year
                                          "none_Fleet_4"),                     # no blocks for trawl fishery
                                      fish_sel_model =                         # fishery selectivity form
                                        c("logist1_Fleet_1",                   # logistic selectivity for fixed gear fleet
                                          "logist1_Fleet_2",
                                          "logist1_Fleet_3",
                                          "logist1_Fleet_4"),                    # gamma function selectivity for trawl gear fleet
                                      fish_q_blocks =                          # fishery indices catchability time blocks
                                        c(
                                          ...
                                          "none_Fleet_2",         # pre-IFQ time block for fixed gear fishery 1994 and before
                                          "none_Fleet_3",         # pre-IFQ time block for fixed gear fishery 1994 and before
                                          "none_Fleet_4"),                     # no blocks for trawl fishery since no CPUE index used
                                      fish_fixed_sel_pars =                    # Whether to estimate all fixed effects for fishery selectivity, but can later modify to fix and share parameters via the parameter mapping
                                        c("est_all", "est_all","est_all","est_all"),               # Estimate all selectivity parameters for both fishery fleets
                                      fish_q_spec =                            # Whether to estimate all fixed effects for fishery catchability
                                        c("fix","fix","fix", "fix"),           # DO NOT Estimate fishery q for fixed gear or for trawl (no CPUE index)
...
input_list <- Setup_Mod_Srvsel_and_Q(input_list = input_list,
                                     ...
                                     srv_sel_model =                           # Survey selectivity form
                                       c("logist1_Block_1_Fleet_1",
                                         "gamma_Block_2_Fleet_1",                      # gamma for BS
...
# CONT SETUP
input_list$map$ln_fish_fixed_sel_pars <- factor(c(1:8,                # all  pars estimated for LLF across all time blocks and both sexes (this is fleet 1, BS)
                                                  9:16,            # same structure for LLF fleet 2 (AI)
                                                  17:24,            # same structure for LLF fleet 3 (GOA)
                                                  rep(25:26,2),                # for trawl fishery estimate both female gamma pars, but no time blocks (repeat 3 times to fill the 3 time blocks associated with the LLF)
                                                  rep(c(27,28),2)))            # for trawl fishery males, share the bmax par of the gamma function with the female estimate par (repeat 3 times to fill the 3 time blocks associated with the LLF)
...
input_list$map$ln_srv_fixed_sel_pars <-
  factor(c(1:8,                                                                # domestic ll survey (2pars, 2 time blocks, 2 sexes), THIS IS FLEET 1--BS
           9:16,                                                               # domestic ll survey (2pars, 2 time blocks, 2 sexes), THIS IS FLEET 2--AI
           17:24,                                                              # domestic ll survey (2pars, 2 time blocks, 2 sexes), THIS IS FLEET 1--BS
...
if(do_francis == 1){
  ...
  st_wts <- data.frame(Comp_Type = c("BS_LLF_Age_F", "BS_LLF_Age_M", "AI_LLF_Age_F", "AI_LLF_Age_M", "GOA_LLF_Age_F", "GOA_LLF_Age_M",
                       ...
                       Init_Weight = c(unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,1] == 1),1,1]), 
                                       ...
                                       unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,2] == 1),1,2]), 
                                       unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,2] == 1),2,2]),
                                       unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,3] == 1),1,3]), 
                                       unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,3] == 1),2,3]),
                                       ...
                                       unique(data$Wt_FishLenComps[1,which(data$UseFishLenComps[,,4] == 1),1,4]), 
                                       unique(data$Wt_FishLenComps[1,which(data$UseFishLenComps[,,4] == 1),2,4]),
  ...
  end_wts <- data.frame(Comp_Type = c("BS_LLF_Age_F", "BS_LLF_Age_M", "AI_LLF_Age_F", "AI_LLF_Age_M", "GOA_LLF_Age_F", "GOA_LLF_Age_M",
                                      "BS_LLS_Age_F","BS_LLS_Age_M", "AI_LLS_Age_F","AI_LLS_Age_M", "GOA_LLS_Age_F","GOA_LLS_Age_M", 
                                      "JPN_LLS_Age_F", "JPN_LLS_Age_M",
                                      "TF_Len_F", "TF_Len_M",
                                      #"BS_LLS_Len_F", "BS_LLS_Len_M","AI_LLS_Len_F", "AI_LLS_Len_M",
...
# Female
# Fits by Year
BS_llf_age_fem <- comp_fits$Fishery_Ages %>%
  ...
  facet_wrap(~Year) +
  theme_sablefish() +
  labs(x = 'Age', y = 'Proportion')+labs(title="BS LLF Age Comps Female")

# Fits by Cohort
BS_llf_age_coh_fem <- comp_fits$Fishery_Ages %>%
  filter(Fleet == 1, Sex == 1) %>%
  mutate(Cohort = Year - Age) %>%
  ggplot() +
  geom_col(aes(x = Age, y = obs), alpha = 0.5) +
  ...
  facet_wrap(~Cohort) +
  theme_sablefish() +
  labs(x = 'Age', y = 'Proportion')+labs(title="BS LLF Age Comps Cohort Fits Female")

# Male
# Fits by Year
if(unique(data$FishAgeComps_Type[,1]) > 0){
  BS_llf_age_mal <- comp_fits$Fishery_Ages %>%
    ...
    facet_wrap(~Year) +
    theme_sablefish() +
    labs(x = 'Age', y = 'Proportion')+labs(title="BS LLF Age Comps Male")
  
  # Fits by Cohort
  BS_llf_age_coh_mal <- comp_fits$Fishery_Ages %>%
    filter(Fleet == 1, Sex == 2) %>%
    mutate(Cohort = Year - Age) %>%
    ggplot() +
    geom_col(aes(x = Age, y = obs), alpha = 0.5) +
    ...
    facet_wrap(~Cohort) +
    theme_sablefish() +
    labs(x = 'Age', y = 'Proportion')+labs(title="BS LLF Age Comps Cohort Fits Male")
}

# AI
# Female
# Fits by Year
AI_llf_age_fem <- comp_fits$Fishery_Ages %>%
  ...
  facet_wrap(~Year) +
  theme_sablefish() +
  labs(x = 'Age', y = 'Proportion')+labs(title="AI LLF Age Comps Female")

# Fits by Cohort
AI_llf_age_coh_fem <- comp_fits$Fishery_Ages %>%
  filter(Fleet == 2, Sex == 1) %>%
  mutate(Cohort = Year - Age) %>%
  ggplot() +
  geom_col(aes(x = Age, y = obs), alpha = 0.5) +
  ...
  facet_wrap(~Cohort) +
  theme_sablefish() +
  labs(x = 'Age', y = 'Proportion')+labs(title="AI LLF Age Comps Cohort Fits Female")

# Male
# Fits by Year
if(unique(data$FishAgeComps_Type[,2]) > 0){
  AI_llf_age_mal <- comp_fits$Fishery_Ages %>%
    ...
    facet_wrap(~Year) +
    theme_sablefish() +
    labs(x = 'Age', y = 'Proportion')+labs(title="AI LLF Age Comps Male")
  
  # Fits by Cohort
  AI_llf_age_coh_mal <- comp_fits$Fishery_Ages %>%
    filter(Fleet == 2, Sex == 2) %>%
    mutate(Cohort = Year - Age) %>%
    ggplot() +
    geom_col(aes(x = Age, y = obs), alpha = 0.5) +
    ...
    facet_wrap(~Cohort) +
    theme_sablefish() +
    labs(x = 'Age', y = 'Proportion')+labs(title="AI LLF Age Comps Cohort Fits Male")
}

# GOA
# Female
# Fits by Year
GOA_llf_age_fem <- comp_fits$Fishery_Ages %>%
  ...
  facet_wrap(~Year) +
  theme_sablefish() +
  labs(x = 'Age', y = 'Proportion')+labs(title="GOA LLF Age Comps Female")

# Fits by Cohort
GOA_llf_age_coh_fem <- comp_fits$Fishery_Ages %>%
  filter(Fleet == 3, Sex == 1) %>%
  mutate(Cohort = Year - Age) %>%
  ggplot() +
  geom_col(aes(x = Age, y = obs), alpha = 0.5) +
  ...
  facet_wrap(~Cohort) +
  theme_sablefish() +
  labs(x = 'Age', y = 'Proportion')+labs(title="GOA LLF Age Comps Cohort Fits Female")

# Male
# Fits by Year
if(unique(data$FishAgeComps_Type[,3]) > 0){
  GOA_llf_age_mal <- comp_fits$Fishery_Ages %>%
    ...
    facet_wrap(~Year) +
    theme_sablefish() +
    labs(x = 'Age', y = 'Proportion')+labs(title="GOA LLF Age Comps Male")
  
  # Fits by Cohort
  GOA_llf_age_coh_mal <- comp_fits$Fishery_Ages %>%
    filter(Fleet == 3, Sex == 2) %>%
    mutate(Cohort = Year - Age) %>%
    ggplot() +
    geom_col(aes(x = Age, y = obs), alpha = 0.5) +
    ...
    facet_wrap(~Cohort) +
    theme_sablefish() +
    labs(x = 'Age', y = 'Proportion')+labs(title="GOA LLF Age Comps Cohort Fits Male")
}
...
if(any(data$UseFishLenComps[,,1] == 1)){  
  BS_llf_len_fem <- comp_fits$Fishery_Lens %>%
    filter(Fleet == 1, Sex == 1) %>%
    ggplot() +
    ...
    facet_wrap(~Year) +
    theme_sablefish() +
    labs(x = 'Length', y = 'Proportion')+labs(title="BS LLF Length Comps Female")
  
  # Fits by Year - Fixed-Gear, Males
  BS_llf_len_male <- comp_fits$Fishery_Lens %>%
    filter(Fleet == 1, Sex == 2) %>%
    ggplot() +
    geom_col(aes(x = Len, y = obs), alpha = 0.5) +
    geom_line(aes(x = Len, y = pred)) +
    facet_wrap(~Year) +
    theme_sablefish() +
    labs(x = 'Length', y = 'Proportion')+labs(title="BS LLF Length Comps Male")
}
#AI
# Fits by Year - Fixed-Gear, Females
if(any(data$UseFishLenComps[,,2] == 1)){  
  AI_llf_len_fem <- comp_fits$Fishery_Lens %>%
    filter(Fleet == 2, Sex == 1) %>%
    ggplot() +
    geom_col(aes(x = Len, y = obs), alpha = 0.5) +
    geom_line(aes(x = Len, y = pred)) +
    facet_wrap(~Year) +
    theme_sablefish() +
    labs(x = 'Length', y = 'Proportion')+labs(title="AI LLF Length Comps Female")
  
  # Fits by Year - Fixed-Gear, Males
  AI_llf_len_male <- comp_fits$Fishery_Lens %>%
    filter(Fleet == 2, Sex == 2) %>%
    ggplot() +
    geom_col(aes(x = Len, y = obs), alpha = 0.5) +
    geom_line(aes(x = Len, y = pred)) +
    facet_wrap(~Year) +
    theme_sablefish() +
    labs(x = 'Length', y = 'Proportion')+labs(title="AI LLF Length Comps Male")
}

#GOA
# Fits by Year - Fixed-Gear, Females
if(any(data$UseFishLenComps[,,3] == 1)){  
  GOA_llf_len_fem <- comp_fits$Fishery_Lens %>%
    filter(Fleet == 3, Sex == 1) %>%
    ggplot() +
    geom_col(aes(x = Len, y = obs), alpha = 0.5) +
    geom_line(aes(x = Len, y = pred)) +
    facet_wrap(~Year) +
    theme_sablefish() +
    labs(x = 'Length', y = 'Proportion')+labs(title="GOA LLF Length Comps Female")
  
  # Fits by Year - Fixed-Gear, Males
  GOA_llf_len_male <- comp_fits$Fishery_Lens %>%
    filter(Fleet == 3, Sex == 2) %>%
    ggplot() +
    geom_col(aes(x = Len, y = obs), alpha = 0.5) +
    geom_line(aes(x = Len, y = pred)) +
    facet_wrap(~Year) +
    theme_sablefish() +
    labs(x = 'Length', y = 'Proportion')+labs(title="GOA LLF Length Comps Male")
}
...
if(do_OSA == 1){
  ...
  ### Fishery Ages ------------------------------------------------------------
  #### BS Domestic Fishery LL Ages -------------------------------------------------
  
  if(unique(data$FishAgeComps_Type[,1]) == 2){                                   # ==0 is aggregated comps, ==1 is split comps, ==2 is joint comps
    osa_results_llfa_BS <- SPoRC::get_osa(obs_mat = comp_fits$Obs_FishAge_mat + 1e-4,
  ...
  resid_plot_llfa_BS <- SPoRC::plot_resids(osa_results = osa_results_llfa_BS)
  
  # Get Aggregated Plot
  fishage_agg_BS <- comp_fits$Fishery_Ages %>%
  ...
  osa_llf_age_BS <- cowplot::plot_grid(resid_plot_llfa_BS[[2]], # Bubble
                                       cowplot::plot_grid(resid_plot_llfa_BS[[1]], fishage_agg_BS, nrow = 1), # QQ and Aggregated
                                       ncol = 1, rel_heights = c(0.6, 0.4), labels = "BS LLF OSA Age Comps")
  
  
  #### AI Domestic Fishery LL Ages -------------------------------------------------
  
  if(unique(data$FishAgeComps_Type[,2]) == 2){                                   # ==0 is aggregated comps, ==1 is split comps, ==2 is joint comps
    osa_results_llfa_AI <- SPoRC::get_osa(obs_mat = comp_fits$Obs_FishAge_mat + 1e-4,
                                          exp_mat = comp_fits$Pred_FishAge_mat + 1e-4,
                                          N = array(c(na.omit(data$ISS_FishAgeComps[1,which(data$UseFishAgeComps[,,2] == 1),1,2] *
                                                                unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,2] == 1),1,2]))),
                                                    dim = c(data$n_regions, length(which(data$UseFishAgeComps[,,2] == 1)))),
                                          years = which(data$UseFishAgeComps[,,2] == 1),
                                          fleet = 2,
                                          bins = ages,
                                          comp_type = 2,                             
                                          bin_label = "Ages")
  ...
  # Get SDNR QQ plot and Bubble Plot
  resid_plot_llfa_AI <- SPoRC::plot_resids(osa_results = osa_results_llfa_AI)
  
  # Get Aggregated Plot
  fishage_agg_AI <- comp_fits$Fishery_Ages %>%
    group_by(Region, Age, Sex, Fleet) %>%
    summarize(obs = mean(obs), pred = mean(pred)) %>%
    filter(Fleet == 2) %>%
    ggplot() +
  ...
  osa_llf_age_AI <- cowplot::plot_grid(resid_plot_llfa_AI[[2]], # Bubble
                                       cowplot::plot_grid(resid_plot_llfa_AI[[1]], fishage_agg_AI, nrow = 1), # QQ and Aggregated
                                       ncol = 1, rel_heights = c(0.6, 0.4), labels = "AI LLF OSA Age Comps")
  
  
  
  #### GOA Domestic Fishery LL Ages -------------------------------------------------
  
  if(unique(data$FishAgeComps_Type[,3]) == 2){
    osa_results_llfa_GOA <- SPoRC::get_osa(obs_mat = comp_fits$Obs_FishAge_mat + 1e-4,
                                           exp_mat = comp_fits$Pred_FishAge_mat + 1e-4,
                                           N = array(c(na.omit(data$ISS_FishAgeComps[1,which(data$UseFishAgeComps[,,3] == 1),1,3] *
                                                                 unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,3] == 1),1,3]))),
                                                     dim = c(data$n_regions, length(which(data$UseFishAgeComps[,,3] == 1)))),
                                           years = which(data$UseFishAgeComps[,,3] == 1),
                                           fleet = 3,
                                           bins = ages,
  ...
  # Get SDNR QQ plot and Bubble Plot
  resid_plot_llfa_GOA <- SPoRC::plot_resids(osa_results = osa_results_llfa_GOA)
  
  # Get Aggregated Plot
  fishage_agg_GOA <- comp_fits$Fishery_Ages %>%
    group_by(Region, Age, Sex, Fleet) %>%
    summarize(obs = mean(obs), pred = mean(pred)) %>%
    filter(Fleet == 3) %>%
    ggplot() +
  ...
  osa_llf_age_GOA <- cowplot::plot_grid(resid_plot_llfa_GOA[[2]], # Bubble
                                        cowplot::plot_grid(resid_plot_llfa_GOA[[1]], fishage_agg_GOA, nrow = 1), # QQ and Aggregated
                                        ncol = 1, rel_heights = c(0.6, 0.4), labels = "GOA LLF OSA Age Comps")
  ...
  if(any(data$UseFishLenComps[,,1] == 1)){
    if(unique(data$FishLenComps_Type[,1]) == 2){
      osa_results_llfl_BS <- SPoRC::get_osa(obs_mat = comp_fits$Obs_FishLen_mat + 1e-4,
    ...
    # Get SDNR QQ plot and Bubble Plot
    resid_plot_llfl_BS <- SPoRC::plot_resids(osa_results = osa_results_llfl_BS)
    
    # Get Aggregated Plot
    fishlens_agg_BS <- comp_fits$Fishery_Lens %>%
    ...
    osa_llf_len_BS <- cowplot::plot_grid(resid_plot_llfl_BS[[2]], # Bubble
                                         cowplot::plot_grid(resid_plot_llfl_BS[[1]], fishlens_agg_BS, nrow = 1), # QQ and Aggregated
                                         ncol = 1, rel_heights = c(0.6, 0.4), labels = "BS LLF OSA Length Comps")
  }
  ...
  if(any(data$UseFishLenComps[,,2] == 1)){
    if(unique(data$FishLenComps_Type[,2]) == 2){
      osa_results_llfl_AI <- SPoRC::get_osa(obs_mat = comp_fits$Obs_FishLen_mat + 1e-4,
                                            exp_mat = comp_fits$Pred_FishLen_mat + 1e-4,
    ...
    # Get SDNR QQ plot and Bubble Plot
    resid_plot_llfl_AI <- SPoRC::plot_resids(osa_results = osa_results_llfl_AI)
    
    # Get Aggregated Plot
    fishlens_agg_AI <- comp_fits$Fishery_Lens %>%
      group_by(Region, Len, Sex, Fleet) %>%
      summarize(obs = mean(obs), pred = mean(pred)) %>%
      filter(Fleet == 2) %>%
      ggplot() +
    ...
    osa_llf_len_AI <- cowplot::plot_grid(resid_plot_llfl_AI[[2]], # Bubble
                                         cowplot::plot_grid(resid_plot_llfl_AI[[1]], fishlens_agg_AI, nrow = 1), # QQ and Aggregated
                                         ncol = 1, rel_heights = c(0.6, 0.4), labels = "AI LLF OSA Length Comps")
  }  
  
  # GOA
  
  if(any(data$UseFishLenComps[,,3] == 1)){
    if(unique(data$FishLenComps_Type[,3]) == 2){
      osa_results_llfl_GOA <- SPoRC::get_osa(obs_mat = comp_fits$Obs_FishLen_mat + 1e-4,
                                             exp_mat = comp_fits$Pred_FishLen_mat + 1e-4,
                                             N = array(c(na.omit(data$ISS_FishLenComps[1,which(data$UseFishLenComps[,,3] == 1),1,3] *
                                                                   unique(data$Wt_FishLenComps[1,which(data$UseFishLenComps[,,3] == 1),1,3]))),
                                                       dim = c(data$n_regions, length(which(data$UseFishLenComps[,,3] == 1)))),
                                             years = which(data$UseFishLenComps[,,3] == 1),
                                             fleet = 3,
                                             bins = lens,
                                             comp_type = 2,
                                             bin_label = "Lengths (cm)")
    ...
    # Get SDNR QQ plot and Bubble Plot
    resid_plot_llfl_GOA <- SPoRC::plot_resids(osa_results = osa_results_llfl_GOA)
    
    # Get Aggregated Plot
    fishlens_agg_GOA <- comp_fits$Fishery_Lens %>%
      group_by(Region, Len, Sex, Fleet) %>%
      summarize(obs = mean(obs), pred = mean(pred)) %>%
      filter(Fleet == 3) %>%
      ggplot() +
    ...
    osa_llf_len_GOA <- cowplot::plot_grid(resid_plot_llfl_GOA[[2]], # Bubble
                                          cowplot::plot_grid(resid_plot_llfl_GOA[[1]], fishlens_agg_GOA, nrow = 1), # QQ and Aggregated
                                          ncol = 1, rel_heights = c(0.6, 0.4), labels = "GOA LLF OSA Length Comps")
  ...
  if(unique(data$FishLenComps_Type[,4]) == 2){
    osa_results_tf <- SPoRC::get_osa(obs_mat = comp_fits$Obs_FishLen_mat + 1e-4,
                                     exp_mat = comp_fits$Pred_FishLen_mat + 1e-4,
                                     N = array(c(na.omit(data$ISS_FishLenComps[1,which(data$UseFishLenComps[,,4] == 1),1,4] *
                                                           unique(data$Wt_FishLenComps[1,which(data$UseFishLenComps[,,4] == 1),1,4]))),
                                               dim = c(data$n_regions, length(which(data$UseFishLenComps[,,4] == 1)))),
                                     years = which(data$UseFishLenComps[,,4] == 1),
                                     fleet = 4,
                                     bins = lens,
                                     comp_type = 2,
                                     bin_label = "Lengths (cm)")
...
if(do_diags == 1){
  ...
  retro <- SPoRC::do_retrospective(n_retro = 7,                                  # number of retro peels to run
                                   ...
                                   n_cores = 8,                                  # if parallel, number of cores to use
...
if(do_high_res_comps == 1){
  ...
  tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_BS_llf_age_fem.png")),units = "in", width=12,height=12, res = 300)
  print(BS_llf_age_fem)
  dev.off()
  
  tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_BS_llf_age_cohort_fem.png")),units = "in", width=12,height=12, res = 300)
  print(BS_llf_age_coh_fem)
  dev.off()
  ...
  if(unique(data$FishAgeComps_Type[,1]) > 0){
    ...
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_BS_llf_age_male.png")),units = "in", width=12,height=12, res = 300)
    print(BS_llf_age_mal)
    dev.off()
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_BS_llf_age_cohort_male.png")),units = "in", width=12,height=12, res = 300)
    print(BS_llf_age_coh_mal)
    dev.off()
  }
  
  tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_AI_llf_age_fem.png")),units = "in", width=12,height=12, res = 300)
  print(AI_llf_age_fem)
  dev.off()
  
  tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_AI_llf_age_cohort_fem.png")),units = "in", width=12,height=12, res = 300)
  print(AI_llf_age_coh_fem)
  dev.off()
  ...
  if(unique(data$FishAgeComps_Type[,2]) > 0){
    ...
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_AI_llf_age_male.png")),units = "in", width=12,height=12, res = 300)
    print(AI_llf_age_mal)
    dev.off()
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_AI_llf_age_cohort_male.png")),units = "in", width=12,height=12, res = 300)
    print(AI_llf_age_coh_mal)
    dev.off()
  }
  
  
  tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_GOA_llf_age_fem.png")),units = "in", width=12,height=12, res = 300)
  print(GOA_llf_age_fem)
  dev.off()
  
  tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_GOA_llf_age_cohort_fem.png")),units = "in", width=12,height=12, res = 300)
  print(GOA_llf_age_coh_fem)
  dev.off()
  ...
  if(unique(data$FishAgeComps_Type[,3]) > 0){
    ...
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_GOA_llf_age_male.png")),units = "in", width=12,height=12, res = 300)
    print(GOA_llf_age_mal)
    dev.off()
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_GOA_llf_age_cohort_male.png")),units = "in", width=12,height=12, res = 300)
    print(GOA_llf_age_coh_mal)
    dev.off()
  }
  
  if(any(data$UseFishLenComps[,,1] == 1)){
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_BS_llf_len_fem.png")),units = "in", width=12,height=12, res = 300)
    print(BS_llf_len_fem)
    dev.off()
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_BS_llf_len_male.png")),units = "in", width=12,height=12, res = 300)
    print(BS_llf_len_male)
    dev.off()
  }
  
  if(any(data$UseFishLenComps[,,2] == 1)){
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_AI_llf_len_fem.png")),units = "in", width=12,height=12, res = 300)
    print(AI_llf_len_fem)
    dev.off()
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_AI_llf_len_male.png")),units = "in", width=12,height=12, res = 300)
    print(AI_llf_len_male)
    dev.off()
  }
  
  if(any(data$UseFishLenComps[,,3] == 1)){
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_GOA_llf_len_fem.png")),units = "in", width=12,height=12, res = 300)
    print(GOA_llf_len_fem)
    dev.off()
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_GOA_llf_len_male.png")),units = "in", width=12,height=12, res = 300)
    print(AI_llf_len_male)
    dev.off()
  }
  ...
  # OSA COMPS
  if(do_OSA == 1){
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_osa_llf_age_BS.png")),units = "in", width=12,height=12, res = 300)
    print(osa_llf_age_BS)
    dev.off()
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_osa_llf_age_AI.png")),units = "in", width=12,height=12, res = 300)
    print(osa_llf_age_AI)
    dev.off()
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_osa_llf_age_GOA.png")),units = "in", width=12,height=12, res = 300)
    print(osa_llf_age_GOA)
    dev.off()
    
    if(any(data$UseFishLenComps[,,1] == 1)){
      tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_osa_llf_len_BS.png")),units = "in", width=12,height=12, res = 300)
      print(osa_llf_len_BS)
      dev.off()
    }
    
    if(any(data$UseFishLenComps[,,2] == 1)){
      tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_osa_llf_len_AI.png")),units = "in", width=12,height=12, res = 300)
      print(osa_llf_len_AI)
      dev.off()
    }
    
    if(any(data$UseFishLenComps[,,3] == 1)){
      tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_osa_llf_len_GOA.png")),units = "in", width=12,height=12, res = 300)
      print(osa_llf_len_GOA)
      dev.off()
    }
...
if(do_pearson == 1){
  ...
  comp_type = 2
  bin_label = "Ages"
  data_source = "BS Fixed Gear Fishery Age Compositions"
  ...
  bs_llf_age_pear <- pearson_stuff[[2]]
  
  tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_Pearson_Resids_BS_LLF_Age.png")),units = "in", width=12,height=8, res = 300)
  print(bs_llf_age_pear)
  dev.off()
  ...
  if(any(data$UseFishLenComps[,,1] == 1)){
    ...
    comp_type = 2
    bin_label = "Lengths"
    data_source = "BS Fixed Gear Fishery Length Compositions"
    ...
    bs_llf_len_pear <- pearson_stuff[[2]]
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_Pearson_Resids_BS_LLF_Length.png")),units = "in", width=12,height=8, res = 300)
    print(bs_llf_len_pear)
    dev.off()
  }
  ...
  comp_type = 2
  bin_label = "Ages"
  data_source = "AI Fixed Gear Fishery Age Compositions"
  ...
  ai_llf_age_pear <- pearson_stuff[[2]]
  
  tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_Pearson_Resids_AI_LLF_Age.png")),units = "in", width=12,height=8, res = 300)
  print(ai_llf_age_pear)
  dev.off()
  ...
  if(any(data$UseFishLenComps[,,2] == 1)){
    ...
    comp_type = 2
    bin_label = "Lengths"
    data_source = "AI Fixed Gear Fishery Length Compositions"
    ...
    ai_llf_len_pear <- pearson_stuff[[2]]
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_Pearson_Resids_AI_LLF_Length.png")),units = "in", width=12,height=8, res = 300)
    print(ai_llf_len_pear)
    dev.off()
  }
  ...
  comp_type = 2
  bin_label = "Ages"
  data_source = "GOA Fixed Gear Fishery Age Compositions"
  ...
  goa_llf_age_pear <- pearson_stuff[[2]]
  
  tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_Pearson_Resids_GOA_LLF_Age.png")),units = "in", width=12,height=8, res = 300)
  print(goa_llf_age_pear)
  dev.off()
  ...
  if(any(data$UseFishLenComps[,,3] == 1)){
    ...
    comp_type = 2
    bin_label = "Lengths"
    data_source = "GOA Fixed Gear Fishery Length Compositions"
    ...
    goa_llf_len_pear <- pearson_stuff[[2]]
    
    tiff(filename=here(root_folder,mod_name,"plots",paste0(mod_name,"_Pearson_Resids_GOA_LLF_Length.png")),units = "in", width=12,height=8, res = 300)
    print(goa_llf_len_pear)
    dev.off()
  }
...
ISS_BS_LLF_age = c(na.omit(data$ISS_FishAgeComps[1,which(data$UseFishAgeComps[,,1] == 1),1,1]))

wt_BS_LLF_age = c(na.omit(unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,1] == 1),1,1])))

ESS_BS_LLF_age = c(na.omit(data$ISS_FishAgeComps[1,which(data$UseFishAgeComps[,,1] == 1),1,1] * 
                             unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,1] == 1),1,1])))

ESS_tab_BS_LLF_Age <- data.frame(ISS = ISS_BS_LLF_age, Data_Wt = rep(wt_BS_LLF_age,length(ISS_BS_LLF_age)), ESS = ESS_BS_LLF_age) 
write.csv(ESS_tab_BS_LLF_Age, here(root_folder,mod_name,"plots",paste0(mod_name,"_BS_LLF_ESS_Age.csv")))

ESS_tab_BS_LLF_Age_mean <- ESS_tab_BS_LLF_Age %>% summarize(ISS = mean(ISS), Data_Wt = mean(Data_Wt), ESS = mean(ESS)) %>% 
  mutate(Source = "BS Fixed Gear Fish Ages") %>% dplyr::relocate(Source) %>%
  mutate(dplyr::across(where(is.numeric), round, 2))

ISS_AI_LLF_age = c(na.omit(data$ISS_FishAgeComps[1,which(data$UseFishAgeComps[,,2] == 1),1,2]))

wt_AI_LLF_age = c(na.omit(unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,2] == 1),1,2])))

ESS_AI_LLF_age = c(na.omit(data$ISS_FishAgeComps[1,which(data$UseFishAgeComps[,,2] == 1),1,2] * 
                             unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,2] == 1),1,2])))

ESS_tab_AI_LLF_Age <- data.frame(ISS = ISS_AI_LLF_age, Data_Wt = rep(wt_AI_LLF_age,length(ISS_AI_LLF_age)), ESS = ESS_AI_LLF_age) 
write.csv(ESS_tab_AI_LLF_Age, here(root_folder,mod_name,"plots",paste0(mod_name,"_AI_LLF_ESS_Age.csv")))

ESS_tab_AI_LLF_Age_mean <- ESS_tab_AI_LLF_Age %>% summarize(ISS = mean(ISS), Data_Wt = mean(Data_Wt), ESS = mean(ESS)) %>% 
  mutate(Source = "AI Fixed Gear Fish Ages") %>% dplyr::relocate(Source) %>%
  mutate(dplyr::across(where(is.numeric), round, 2))

ISS_GOA_LLF_age = c(na.omit(data$ISS_FishAgeComps[1,which(data$UseFishAgeComps[,,3] == 1),1,3]))

wt_GOA_LLF_age = c(na.omit(unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,3] == 1),1,3])))

ESS_GOA_LLF_age = c(na.omit(data$ISS_FishAgeComps[1,which(data$UseFishAgeComps[,,3] == 1),1,3] * 
                              unique(data$Wt_FishAgeComps[1,which(data$UseFishAgeComps[,,3] == 1),1,3])))

ESS_tab_GOA_LLF_Age <- data.frame(ISS = ISS_GOA_LLF_age, Data_Wt = rep(wt_GOA_LLF_age,length(ISS_GOA_LLF_age)), ESS = ESS_GOA_LLF_age) 
write.csv(ESS_tab_GOA_LLF_Age, here(root_folder,mod_name,"plots",paste0(mod_name,"_GOA_LLF_ESS_Age.csv")))

ESS_tab_GOA_LLF_Age_mean <- ESS_tab_GOA_LLF_Age %>% summarize(ISS = mean(ISS), Data_Wt = mean(Data_Wt), ESS = mean(ESS)) %>% 
  mutate(Source = "GOA Fixed Gear Fish Ages") %>% dplyr::relocate(Source) %>%
  mutate(dplyr::across(where(is.numeric), round, 2))

ISS_TF_len = c(na.omit(data$ISS_FishLenComps[1,which(data$UseFishLenComps[,,4] == 1),1,4]))

wt_TF_len = c(na.omit(unique(data$Wt_FishLenComps[1,which(data$UseFishLenComps[,,4] == 1),1,4])))

ESS_TF_len = c(na.omit(data$ISS_FishLenComps[1,which(data$UseFishLenComps[,,4] == 1),1,4] * 
                         unique(data$Wt_FishLenComps[1,which(data$UseFishLenComps[,,4] == 1),1,4])))
...
ESS_tab <- rbind(ESS_tab_BS_LLF_Age_mean, ESS_tab_AI_LLF_Age_mean, ESS_tab_GOA_LLF_Age_mean, 
                 ESS_tab_BS_LLS_Age_mean, ESS_tab_AI_LLS_Age_mean, ESS_tab_GOA_LLS_Age_mean, 
