...
mod_name2 <- c("26.1i_1Reg_Cont_Upd_No_wAI_LLS", "26.2b_FAA_3Flt_LLS_Only","26.2b_FAA_3Flt_LLS_Only_pre_bias_corr")                         # names of model want to compare, these must match the names used for the SPoRC outputs results rdata files

drop_cont <- c(-1)
mods_no_cont <- mod_name2[-1]
comp_name <- "FAA_Build"                                                       # name for saving files

path <- here("Mods","__FAA_mod_build",mod_name2[2],"Mod_Comp")                   # Assumes that comparison folder is inside the first model folder, but can update this to be elsewhere
