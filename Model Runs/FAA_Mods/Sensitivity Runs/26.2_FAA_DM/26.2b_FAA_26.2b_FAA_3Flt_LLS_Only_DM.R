...
mod_name <- '26.2b_FAA_3Flt_LLS_Only_DM'                                                    # Model name to use for saving files and labels
...
input_list <- Setup_Mod_SrvIdx_and_Comps(input_list = input_list,
                                         ...
                                         SrvAgeComps_LikeType =                # age comp likelihoods by survey fleet (none if not fit or not available for given fleet)
                                           c("Dirichlet-Multinomial", "Dirichlet-Multinomial","Dirichlet-Multinomial","none", "Dirichlet-Multinomial"),
...
theta <- exp(sabie_rtmb_model$sd_rep$par.fixed[str_detect(names(sabie_rtmb_model$sd_rep$par.fixed), 'theta')])

(1 + theta[1] * data$ISS_FishAgeComps[,,1,1]) / (1 + theta[1])
(1 + theta[2] * data$ISS_FishLenComps[,,1,2]) / (1 + theta[2])
(1 + theta[3] * data$ISS_SrvAgeComps[,,1,1]) / (1 + theta[3])
(1 + theta[4] * data$ISS_SrvAgeComps[,,1,2]) / (1 + theta[4])
(1 + theta[5] * data$ISS_SrvAgeComps[,,1,3]) / (1 + theta[5])
(1 + theta[6] * data$ISS_SrvAgeComps[,,1,5]) / (1 + theta[6])
...
if(do_OSA == 1){
  ...
  if(unique(data$SrvAgeComps_Type[,3]) == 2){
    osa_results_llsa_GOA <- SPoRC::get_osa(obs_mat = comp_fits$Obs_SrvAge_mat + 1e-4,
                                           ...
                                           bins = ages,
                                           comp_like = 1,
                                           DM_theta = exp(sabie_rtmb_model$optim$par[str_detect(names(sabie_rtmb_model$optim$par), 'theta')])[5],
                                           comp_type = 2,                              # ==0 is aggregated comps, ==1 is split comps, ==2 is joint comps
                                           bin_label = "Ages")
  }
  # Get SDNR QQ plot and Bubble Plot
  ...
  if(unique(data$SrvAgeComps_Type[,5]) == 2){
    osa_results_jpn_llsa <- SPoRC::get_osa(obs_mat = comp_fits$Obs_SrvAge_mat + 1e-4,
                                           ...
                                           fleet = 5,
                                           comp_like = 1,
                                           DM_theta = exp(sabie_rtmb_model$optim$par[str_detect(names(sabie_rtmb_model$optim$par), 'theta')])[6],
                                           bins = ages,
                                           comp_type = 2,                          # ==0 is aggregated comps, ==1 is split comps, ==2 is joint comps
                                           bin_label = "Ages")
  }
...
if(do_diags == 1){
  ...
  retro <- SPoRC::do_retrospective(n_retro = 7,                                  # number of retro peels to run
                                   ...
                                   do_francis = FALSE                            # if we want tod o Francis
  )
