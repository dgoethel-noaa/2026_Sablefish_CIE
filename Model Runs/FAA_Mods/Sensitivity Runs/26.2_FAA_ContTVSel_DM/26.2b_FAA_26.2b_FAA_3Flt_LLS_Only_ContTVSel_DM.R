...
mod_name <- '26.2b_FAA_3Flt_LLS_Only_ContTVSel_DM'                                                    # Model name to use for saving files and labels
...
# Define valid fleet-block combinations
fleet_blocks <- data.frame(
  fleet = c(1,2,3,4,5),                                             # fleets for each time block (LLS has 2 timeb blocks TS and JPN have 1 each)
  block = c(1,1,1,1,1)                                              # corresponding blocks
)
...
input_list <- Setup_Mod_Srvsel_and_Q(input_list = input_list,
                                     ...
                                     srvsel_pe_pars_spec = rep("fix", 5), 
                                     corr_opt_semipar = rep("corr_zero_y_b", 5),
...
theta <- exp(sabie_rtmb_model$optim$par[str_detect(names(sabie_rtmb_model$optim$par), 'theta')])
