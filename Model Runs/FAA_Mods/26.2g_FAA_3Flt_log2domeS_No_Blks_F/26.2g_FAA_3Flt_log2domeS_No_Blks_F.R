...
mod_name <- '26.2g_FAA_3Flt_log2domeS_No_Blks_F'                                                    # Model name to use for saving files and labels
...
fleet_blocks <- data.frame(
  fleet = c(1, 2,3,4),                                           # fleets corresponding to time blocks (3 fixed gear time blocks, no trawl gear blocks)
  block = c(1, 1, 1,   1)                                       # corresponding time blocks
)
...
# CONT SETUP
input_list$map$ln_fish_fixed_sel_pars <- factor(c(1:4,                # all  pars estimated for LLF across 1 time blocks and both sexes (this is fleet 1, BS)
                                                  5:8,            # same structure for LLF fleet 2 (AI)
                                                  9:12,            # same structure for LLF fleet 3 (GOA)
                                                  rep(13:14,1),                # for trawl fishery estimate both female gamma pars, but no time blocks (repeat 3 times to fill the 3 time blocks associated with the LLF)
                                                  rep(c(15,16),1)))            # for trawl fishery males, share the bmax par of the gamma function with the female estimate par (repeat 3 times to fill the 3 time blocks associated with the LLF)
