...
mod_name <- '26.2a_FAA_3Flt_LLS_Idx_Only'                                                    # Model name to use for saving files and labels
...
# DO NOT FIT ANY LENGTH COMPS FOR LLF OR LLS
#SPoRC_one_reg$UseFishLenComps[,,1:3] <- 0
#SPoRC_one_reg$UseSrvLenComps[,,1:3] <- 0
...
# Merge to get all valid combinations    # remove priors for any unestimated pars -- par1=a50, par2=delta; NEEDS TO MATCH PARAMETER MAPPING
srv_selex_structure <- merge(fleet_blocks, sex_par) %>%
  dplyr::filter(!(fleet == 1)) %>%                                           # Estimating a single selex for the LLS (and fitting aggregated comps), so only active selex pars are for GOA LLS       
  dplyr::filter(!(fleet == 2)) %>%                                              
 # dplyr::filter(!(fleet == 5 & block == 1 & sex == 2 & par == 2)) %>%         
  dplyr::filter(!(fleet == 4))                                                # not fitting or estimating trawl survey                                          
...
input_list <- Setup_Mod_Srvsel_and_Q(input_list = input_list,
                                     ...
                                     srv_fixed_sel_pars_spec =                 # Whether to estimate all fixed effects for survey selectivity, but can later modify to fix and share parameters via the parameter mapping
                                       c("est_shared_f_3",                            # Estimate all selectivity parameters for all survey fleets
                                         "est_shared_f_3",
                                         "est_all",
                                         "fix",
                                         "est_all"),
...
# CONT SETUP
input_list$map$ln_fish_fixed_sel_pars <- factor(c(rep(c(1:4),1),               # estimate a50 and a95 for females (2pars), but repeat 2xs for the 2 time blocks
                                                  rep(c(5:8),1),               # estimate a50 and a95 for males (2pars), but repeat 2xs for the 2 time blocks
...
input_list$map$ln_srv_fixed_sel_pars <-
  factor(c(rep(c(1:8),3),                                                      # females +males 2 pars each, 2 blocks (8 estimated pars) (shared across all 3 LLS fleets; only one set of estimated pars....so repeat 3 times for the 3 fleets)    
           ...
           rep(c(11:12),2)))                                                   # coop JPN LL survey (single time block, males), share delta
...
#write.csv(list(sabie_rtmb_model$sd_rep), here(root_folder,mod_name,"plots",paste0(mod_name,"_sd_rep.csv")))
